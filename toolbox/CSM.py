...
class CSM(Argument, Settings):
    ...
    def process(self):
        ...
        self.selectedData = self.nc.createVariable(
            'data', 'f4', ('time', 'lev', 'lat', 'lon'))
        self.dataRows = 0

        # 获取数据
        ncfiles = glob(os.path.join(self.dataPath, '*' +
                                    self.parameters['var'] + '*.nc'))
        if not ncfiles:
            ...
            for filename in ncfiles:
                ...
                self.selectedData[self.dataRows, :, :, :] = np.expand_dims(
                    tmpfile.variables[self.parameters['var']][
                        self.parameters['lt'], :, :, :], axis=0)
                tmpfile.close()

                # 更新时间
                startDate = dt.datetime.strptime(
                    basename.split('.')[0], "%Y%m%d")
                ...
                self.times[
                    self.dataRows] = '-'.join([str(tmpYear), str(tmpMonth)])
    ...
    def cutMean(self):
        ...
        for month in self.monthList:
            self.monthMean.append(
                np.mean(self.selectedData[self.monthList[month], :, :, :],
                        axis=0))
