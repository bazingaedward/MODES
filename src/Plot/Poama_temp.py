...
# latest:2016.10.20
# description: Use Bayes Formular with GPCP and CMA106 model data,with plot result
# data_range: 1991 - 2015 逐年7月份平均降水(mm/day),纬度:40N,经度:115
...
def plot(x, y, zi, title):
    ...
    cs = map.contourf(xx, yy, zi, cmap='YlOrBr')
...
if __name__ == '__main__':
    ...
    for file in glob.glob('201707/*.nc'):
        ...
        x = fh.variables['lon'][:]
        y = fh.variables['lat'][:]
        data.append(fh.variables['tsfmax'][0, :])
    zi = np.mean(np.array(data), axis=0) - 273.15
    plot(x, y, zi, 'MODES_BoM-POAMA_201707_TEM_Mean')